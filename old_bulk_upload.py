    ...
    try:
        ...
        if not ws['A' + str(ini)].value:
            ...
            if saved:
                ...
                quantity = int(ws['AA' + str(ini)].value) or 1
            ...
            if data['original_price'] >= data['listing_price']:
                ...
                if img_ids:
                    ...
                    if ws['AC' + str(ini)].value:
                        data['occasion'] = int(ws['AC' + str(ini)].value)
                    else:
                        data['occasion'] = None

                    data['color'] = int(ws['AE' + str(ini)].value)
                    data['discount'] = (data['original_price'] - data['listing_price']) / data['original_price']
                    data['status'] = '0'
                    data['completed'] = True
                    data['time_to_make'] = ws['AG' + str(ini)].value
                    data['partner_id'] = ws['AH' + str(ini)].value
                    ...
                    if srlzr.is_valid():
                        ...
                        no_pro_data = {'size': int(ws['Z' + str(ini)].value), 'product': product.id,
                                       'quantity': quantity, 'time_to_make': str(ws['AG' + str(ini)].value)}
                        no_pro_srlzr = NumberOfProductSrlzr(data=no_pro_data)
                        if no_pro_srlzr.is_valid():
                            no_pro_srlzr.save()
                        if int(data['user']) in [788, 802]:
        ...
        print 'upload failed in row str(ini)', e
        # pdb.set_trace()
        # print e
        out_ws['A' + str(out_ini)] = ini
